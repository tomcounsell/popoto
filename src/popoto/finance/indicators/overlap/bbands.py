...
class BbandsStorage(IndicatorStorage):
    ...
    def get_width(self):
        ...
        if self.value:
            [self.upperband_val, self.middleband_val, self.lowerband_val] = [
                float(val) for val in self.value.split(":")
            ]
    ...
    def compute_value_with_requisite_indexes(
        ...
        logger.debug(
            f"Bbands computed: {upperband[-1]}:{middleband[-1]}:{lowerband[-1]}"
        )
    ...
    def produce_signal(self):
        ...
        bbands_widths = [
            (float(v.split(":")[upper]) - float(v.split(":")[lower]))
            / float(v.split(":")[middle])
            for v in query_result["values"]
        ]

        if self.width <= min(
            bbands_widths
        ):  # smallest width (squeeze) in the last 180 periods
            # squeeze = True
