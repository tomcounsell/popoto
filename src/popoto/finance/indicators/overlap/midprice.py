...
class MidpriceSubscriber(IndicatorSubscriber):
    ...
    def handle(self, channel, data, *args, **kwargs):
        ...
        if self.index is not "low_price":
            logger.debug(
                f"index {self.index} is not `high_price` or `low_price` ...ignoring..."
            )
            return

        new_midprice_storage = MidpriceStorage(
        ...
        for horizon in HORIZONS:
            ...
            high_value_np_array = new_midprice_storage.get_denoted_price_array(
                "high_price", periods
            )
            low_value_np_array = new_midprice_storage.get_denoted_price_array(
                "low_price", periods
            )
