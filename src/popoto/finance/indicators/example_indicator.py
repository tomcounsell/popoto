...
class ExampleIndicatorStorage(IndicatorStorage):
    ...
    def compute_value_with_requisite_indexes(
        ...
        example_indicator_value = talib.EXAMPLE(
            requisite_pv_index_arrrays[
                "close_price"
            ],  # 'close_price' is set as a requirement on line 17
            requisite_pv_index_arrrays[
                "high_price"
            ],  # 'close_price' is set as a requirement on line 17
            ...
            -1
        ]  # talib will return an array, so just take the last value

        if math.isnan(example_indicator_value):
            return ""  # best practice to return empty string if no value is available

        return str(
            example_indicator_value
        )  # this will save to self.value for each instance
...
class FooStorage(IndicatorStorage):
    ...
    def compute_value_with_requisite_indexes(
        ...
        foo_value = talib.FOO(
            ...
            requisite_pv_index_arrrays["close_volume"],
