...
class UltoscSubscriber(IndicatorSubscriber):
    ...
    def handle(self, channel, data, *args, **kwargs):
        ...
        new_ultosc_storage = UltoscStorage(
        ...
        for horizon in HORIZONS:
            ...
            high_value_np_array = new_ultosc_storage.get_denoted_price_array(
                "high_price", periods
            )
            low_value_np_array = new_ultosc_storage.get_denoted_price_array(
                "low_price", periods
            )
            close_value_np_array = new_ultosc_storage.get_denoted_price_array(
                "close_price", periods
            )

            ultosc_value = talib.ULTOSC(
                high_value_np_array,
                low_value_np_array,
                close_value_np_array,
                timeperiod1=horizon * 7,
                timeperiod2=horizon * 14,
                timeperiod3=horizon * 28,
            )[-1]
