...
class AroonSubscriber(IndicatorSubscriber):
    ...
    def handle(self, channel, data, *args, **kwargs):
        ...
        new_aroon_storage = AroonStorage(
        ...
        for horizon in HORIZONS:
            ...
            high_value_np_array = new_aroon_storage.get_denoted_price_array(
                "high_price", periods
            )
            low_value_np_array = new_aroon_storage.get_denoted_price_array(
                "low_price", periods
            )

            timeperiod = min(
                [len(high_value_np_array), len(low_value_np_array), periods]
            )
            aroondown_value, aroonup_value = talib.AROON(
                high_value_np_array, low_value_np_array, timeperiod=timeperiod
            )[-1]
            ...
            new_aroon_storage.value = ":".join(
                [str(aroondown_value), str(aroonup_value)]
            )
