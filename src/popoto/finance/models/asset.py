...
class AssetModel(OLHCVModel):
    ...
    def query(cls, symbol, publisher="", *args, **kwargs):
        ...
        asset = (
            assets.filter(name__icontains=publisher).first()
            if publisher
            else assets.first()
        )
        if asset:
    ...
    def renew(
    ...
    ):  # get up-to-date with newest data
        from ...redis_db import POPOTO_REDIS_DB

        end_timestamp = end_timestamp or self.timestamp
        one_day_seconds = 24 * 3600
        start_timestamp = start_timestamp or end_timestamp - (
            self.days_range * one_day_seconds
        )
        ...
        if POPOTO_REDIS_DB.ttl(f"throttle:{PUBLISHERS[self.asset_class]}") > 0:
            return
        POPOTO_REDIS_DB.set(
            f"throttle:{PUBLISHERS[self.asset_class]}",
            "slow up bro",
            ex=throttle_timeout,
        )  # set throttle
        ...
        if PUBLISHERS[self.asset_class] == "polygon":
            with PolygonAPI(POLYGON_API_KEY) as polygon_client:
                ...
                polygon_client_response = polygon_client._session.get(
                    endpoint, params={}, timeout=polygon_client.timeout
                )
                logging.debug(
                    f"polygon responded with status: {polygon_client_response.status_code}"
                )

            if (
                polygon_client_response.status_code == 200
                and polygon_client_response.json()["results"]
            ):
                ...
                for aggregate_result in polygon_client_response.json()["results"]:
                    for agg_key, index_name in {
                        ...
                        price_storage = PriceStorage(
                            ...
                            publisher="polygon",
                            index=index_name,
                            timestamp=get_nearest_1hr_timestamp(
                                float(aggregate_result["t"]) / 1000
                            ),
        ...
        if PUBLISHERS[self.asset_class] == "yahoo":
            ...
            for index in set(self.yahoo_data.keys()).intersection(
                {"open", "high", "low", "close"}
            ):
