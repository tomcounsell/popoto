    ...
    def get_dataframe(self, timestamp: int = 0, days_range: int = 0):
        ...
        for index in reversed(DEFAULT_PRICE_INDEXES):
            price_timeseries = PriceStorage.query(
                ...
                periods_range=float(self.days_range)
                * PERIODS_24HR
                * 1.1,  # n days x 24 hours * 110%
            )
            if (
                "scores" not in price_timeseries
                or int(price_timeseries["values_count"]) < 1
            ):
                continue

            dataframes[index] = pd.DataFrame(
                index=[
                    get_nearest_1day_score(score)
                    for score in price_timeseries["scores"]
                ],
                data={
                    index: price_timeseries["values"],
                },
                dtype=float,
            )
        ...
        for index in DEFAULT_VOLUME_INDEXES:
            ...
            if "scores" in volume_timeseries:
                dataframes[index] = pd.DataFrame(
                    index=[
                        get_nearest_1day_score(score)
                        for score in volume_timeseries["scores"]
                    ],
                    data={
        ...
        self.dataframe = pd.DataFrame(
            index=range(
                get_nearest_1day_score(low_score),
        ...
        for index in reversed(DEFAULT_PRICE_INDEXES):  # close_price goes first
            ...
            self.dataframe[index].fillna(
                self.dataframe["close_price"], axis=0, inplace=True
            )
