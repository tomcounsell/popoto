...
- **`ContextAssembler` token budget now enforced for real** ([#408](https://github.com/tomcounsell/popoto/issues/408)):
  ...
  - New default heuristic (`_estimate_tokens`): escape-aware character-class estimator accurate to within ±25% vs tiktoken cl100k_base across English, code, CJK, and emoji content (worst case −15% on URL/hash-heavy records — the safe overestimate direction).
