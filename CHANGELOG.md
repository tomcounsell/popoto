...
- **The harness hook stops after the first connection failure** — one Redis outage no longer costs the user's prompt five sequential 5-second timeouts. The integration binds its connection with a 1-second socket and connect timeout and no redis-py retries, `MemoryService` short-circuits every later operation in that process once a `ConnectionError`/`TimeoutError` has been recorded, and the shipped Claude Code and Codex hook configs carry an explicit 10-second `timeout`.
...
- **Lua scripts are registered once and invoked with `EVALSHA`** — every call site that sent script source with `EVAL` on each invocation (38 of them, including the 7 KB decay scorer on every `top_by_decay`) now goes through `popoto.redis_db.run_lua`, which caches a redis-py `Script` per script text and falls back to loading on `NOSCRIPT`.
- **Competitive suppression is one pipeline** — `ContextAssembler` used to issue `EXISTS` + `EVAL` sequentially for every fetched-but-not-selected candidate on every turn. `ConfidenceField.update_confidence` now honors its `pipeline` argument (the existence check moved into the Lua script), so a turn's suppressions ride the post-effects pipeline. Measured: 25 round trips per `inject_context` down to 8. **Breaking for direct callers that passed `pipeline=`**, where the argument was previously accepted and ignored: in pipeline mode the call now returns `None` instead of the new confidence, does not sync the instance attribute, and skips an unsaved instance server-side rather than raising `TypeError`. Read the value back with `get_confidence` after `execute()`.
- **Field index namespaces derive from the class name suffix** — `FieldBase` used `str.strip("Field")`, which strips a character set, so `FloatField` indexed under `$oatF` and two unrelated class names could share a namespace. New field classes now get `$<Name minus Field>F`; the fourteen shipped classes whose keys are already on disk under the old spelling pin it explicitly, so no key Popoto itself writes changes. **A field class you wrote yourself is not pinned.** If its name ends in any of `F`/`i`/`e`/`l`/`d` beyond the `Field` suffix, its namespace moves on upgrade — `MyCacheField` was `$MyCachF` and becomes `$MyCacheF` — and its existing keys stay on disk unread. Pin the old spelling with `field_class_key = DB_key("$MyCachF")` before upgrading, or migrate the keys. See [Writing Custom Fields](docs/field-authoring.md).
...
- **`Meta.ttl` no longer leaves permanent index ghosts** — `EXPIRE` applies to the hash only, so after expiry the class set, key-field pointer sets and sorted-set indexes still held the member and every query logged an error pointing at a `repair_indexes()` that did not exist. A read that finds a member without a hash now purges it from every index derivable from the key alone (class set, non-auto `KeyField` sets, sorted sets partitioned by key fields) in one pipeline, and the message names `clean_indexes()` for the rest.
