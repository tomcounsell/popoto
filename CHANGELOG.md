...
- **`datetime` and `time` values no longer lose their UTC offset in storage** ([#521](https://github.com/tomcounsell/popoto/issues/521)). The encoder used `%Y%m%dT%H:%M:%S.%f`, which has no offset directive: a timezone-aware value went in and a naive one came out, and `12:00+07:00` and `12:00+00:00` produced byte-identical storage. This was the root cause underneath #519 — that fix made the *score* consistent, this one makes the *value* correct. Both types now encode with `isoformat()`, which carries the offset when the value is aware and omits it when the value is naive, so awareness round-trips in both directions.
  ...
  - **Rows written before this release still read, and are assumed to be UTC.** The decoder accepts both shapes and stamps `timezone.utc` on the legacy offset-free form, which agrees with #519's naive-as-UTC scoring and with #421. **This is an assumption, not a recovery.** A value originally written by a process in a non-UTC zone held a different instant, and its offset was never stored, so it is unrecoverable here or anywhere else — such values are now labelled UTC while carrying a local wall clock. **No offline migration is required** (and none is possible), but if your pre-upgrade writers were not in UTC, audit before upgrading.
  - **Legacy rows now compare as aware.** Code comparing a pre-upgrade value against a naive `datetime.now()` will raise `TypeError` where it previously compared. Use `datetime.now(timezone.utc)`. Post-upgrade values written deliberately naive stay naive and are unaffected — the two offset-free shapes are distinguishable, because the legacy format has no date separators.
  - **`time` is excluded from the UTC assumption, and cannot be included.** `%H:%M:%S.%f` and `time.isoformat()` of a naive time with microseconds are byte-identical, so a legacy value is indistinguishable from one deliberately written naive after this release. Legacy times therefore stay naive. Nothing downstream depends on it: `convert_to_numeric` ignores `tzinfo` when scoring a time.
