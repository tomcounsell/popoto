...
- **`limit` is now pushed into the sorted set read when that is provably equivalent.** A query that filters a `SortedField` by range, orders by that same field, and constrains nothing else beyond the field's own `partition_by` values now issues `ZRANGE ... BYSCORE LIMIT 0 <limit>` instead of an unbounded `ZRANGEBYSCORE` followed by a Python-side sort and slice. Hydration drops from every member of the range to `limit` members, which is the difference between O(range) and O(limit) `HGETALL`s on a hot partition. The gate is conservative by construction: a predicate on any other field, an unindexed field applied client-side, a second `SortedField`, `Q` objects, ordering by a different field, or a non-positive limit all fall back to the previous unbounded read, byte for byte. This is required — truncating the range read when a later predicate can still drop rows would silently return fewer than `limit` valid results. `count()` is unaffected and still reports the full match count. See `Query._resolve_range_pushdown` for the exact condition.
