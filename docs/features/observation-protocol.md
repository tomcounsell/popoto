...
- an unsaved member is skipped for that effect: it never raises out of
  `on_context_used`, and it never aborts the batch;
- every other, saved instance in the same call still gets its full effects, applied
  through the same pipeline and `execute()`d as normal;
- the model methods themselves are unchanged — calling `touch()`,
  `confirm_access()`, `strengthen_cycle()`, `weaken_cycle()`, or
  `resolve_pressure()` directly on an unsaved instance still raises `TypeError`,
  exactly as before. The degradation is a property of `ObservationProtocol`, not of
  the model methods it calls — a caller invoking those methods outside the protocol
  gets no such safety net.
