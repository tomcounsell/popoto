...
**An `anthropic` that is installed but too old raises too, at construction.** The provider passes `output_config=` to `messages.create`, which no release before **0.77.0** accepts, and `popoto[anthropic]` floors the package there. If some other pin puts an older SDK in the environment anyway, constructing `ClaudeExtractionProvider` raises `AnthropicVersionError` (`popoto.extraction.AnthropicVersionError`, a `RuntimeError` subclass) naming the missing parameter and the required version. It is deliberately distinct from the `ImportError` above, because "not installed" and "installed but too old" need different remedies. The check runs at construction rather than inside `extract()` for the reason the next paragraph makes clear: there, a version mismatch would be swallowed and returned as an empty fact list, indistinguishable from "this text contained no facts" ([#670](https://github.com/tomcounsell/popoto/issues/670)).
