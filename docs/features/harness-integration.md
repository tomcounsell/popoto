...
**The read→write handoff is keyed on the harness's turn identifier.**
`MemoryService` queues the selected record keys per `session_id` and tags
each entry with the turn id the payload carried — Claude Code's `prompt_id`,
Codex's `turn_id`. The write event claims its own entry by that tag rather
than popping whatever sits at the head, so an aborted turn, a crashed
session, or a `SubagentStop`-configured session reporting more often than it
read no longer shifts every later pairing by one. A second report for a turn
...
| `POPOTO_MEMORY_TURN_KEYED` | `1` | `0` restores the pre-1.9 session-wide FIFO handoff, pairing reads to writes by queue position instead of by turn id |
