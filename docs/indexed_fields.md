...
pointer key itself is namespaced under `$IdxPtr:`, alongside Popoto's other
internal keys (`$Class:`, `$KeyF:`, `$SortedF:`), and follows the pattern:

```
$IdxPtr:<Model:key>:<field_name>  ->  $IndexF:ModelName:field_name:current_value
```

A model name can never begin with `$`, so this key sits outside every model's
own key space and can never collide with a model key glob (e.g. the
`AutoKeyField` lookup path, which resolves via a `scan_keys()` glob).
...
!!! note "Versions prior to this fix (1.8.0-1.8.2, see issues #476, #540)"
    ...
    The 1.8.1/1.8.2 fix for that moved the pointer to a side key, but derived
    it by suffixing the model hash key itself
    (`<Model:key>\x00idxptr\x00<field_name>`) rather than namespacing it under
    `$IdxPtr:`. A NUL byte does not keep a key out of a glob — Redis glob `*`
    matches any byte, including NUL — so that key was matched by the same
    glob `AutoKeyField` lookups scan, and the follow-up `HGETALL` failed with
    `WRONGTYPE`. This regressed `get_by_id()`-style AutoKey lookups to return
    nothing for rows that exist (#540).

    Current code never writes to either legacy location. Records written by
    an affected 1.8.0-1.8.2 release are read as a migration fallback and
    self-heal (the legacy pointer is scrubbed and moves to the `$IdxPtr:`
    side key) the next time they're saved. During a rolling upgrade, a
    legacy pointer still visible to a not-yet-upgraded node is skipped by a
    type guard on the scan path rather than crashing the read. No offline
    migration step is required; a full clean cut-over is optional:
