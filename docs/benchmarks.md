...
**Model class:** `ExtMem<hash>` — a fresh Popoto Model per benchmark item,
built with `type(class_name, bases, namespace_dict)` so the metaclass captures
the per-item name (and therefore `_meta.db_class_key`, from which every field
derives its Redis keys) at class creation. Fields:
