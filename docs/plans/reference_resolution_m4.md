...
**Round 1 — 2026-09-04, FULL depth** (`appetite: Large` → deterministic force-FULL). Lenses:
Risk & Robustness, Scope & Value, History & Consistency, plus automated structural checks.
**Verdict: NEEDS REVISION — 4 blockers.**

| Severity | Critic | Finding | Addressed By | Implementation Note |
|----------|--------|---------|--------------|---------------------|
| BLOCKER | Structural + History & Consistency | Verification row `python -m mypy src/` → "exit code 0" is unsatisfiable: the gate fails at baseline `ab4762c` with **1178 errors in 71 files**. Success Criteria already state the correct rule ("no new errors versus base"), so the two sections disagree and the table row can never be met. | | Replace the row's expected value with a **delta** measurement, not an exit code: capture `python -m mypy src/ 2>&1 \| tail -1` on base and on the branch **in the same redis-py environment** and require the branch count ≤ base count. Per CLAUDE.md the delta is redis-py-version-dependent (7.x flags sites 8.x narrows), so state the redis-py version alongside the number. |
| BLOCKER | Structural + History & Consistency | Verification row `python -m pytest -q` → "exit code 0" contradicts the note directly beneath the same table: "The six tests that fail by construction on a non-15 DB are expected noise" — while the table simultaneously mandates a non-15 DB for every pytest row. The full-suite gate is therefore unpassable as written. | | Change the expected value to "exit code 0 **except** the six known non-15-DB failures, named individually in the run report". Enumerate that set once (`grep -rn "15" tests/test_pytest_plugin.py` and `docs/sdlc/do-sdlc.md`) and pin the names in the plan so the validator asserts an exact allowlist rather than a fuzzy count. |
| BLOCKER | Risk & Robustness (Adversary) | The fail-open contract has a hole at the `journal.append(at=…)` boundary. `_coerce_instant` (`provenance_journal.py:1266-1283`) **raises `ValueError`** on a non-finite or non-numeric `at`; that raise lands in `_append_and_transition`'s generic `except Exception` (`decision_log.py:795-812`), which writes `reject(ASSEMBLY_FAILED)` and returns `None` — **the candidate is not captured at all**. This directly contradicts "quality loss, not corruption" and the Failure Path bullet claiming `NaN`/`inf` are "coerced to the clock, matching `_coerce_instant`'s rejection semantics" (they are not coerced; they raise). | | Validate in `resolution.py`, never at the journal boundary: `if valid_from is not None and not math.isfinite(valid_from): valid_from = None` before it is placed on `Resolution`, and the same guard on `TurnContext.captured_at` (note `_write` does a bare `float(captured_at)` at `provenance_journal.py:930` and will happily store a `NaN` without raising, so `captured_at` fails *silently* while `at` fails *loudly* — two different bugs from one bad float). Add a test asserting a `Resolution` carrying `float("nan")` still produces a journal entry. |
| BLOCKER | History & Consistency (Consistency Auditor) | Open Question 4 is marked **"needs input"** and states the plan "deliberately narrows a written requirement" of the 2026-08-16 issue amendment (which names *both* "since March" and "by Friday" as `valid_from` producers) — yet that unconfirmed narrowing is already hard-committed as Success Criterion "An onset reference emits `valid_from`; a deadline reference does not", as Technical Approach §4, as Risk 3's mitigation, and the plan frontmatter is `status: Ready`. Q5 and Q6 are likewise open and likewise pre-decided. | | Do not resolve this in the plan text — it is a product decision. Either (a) get the maintainer's confirmation and record it in the Amendment/Open Questions section with a date, or (b) if building before confirmation, demote the onset rule to a pinned constant so a reversal is one edit: `Defaults.M4_VALID_FROM_ROLES = ("onset",)` read by the emission rule, with a test parameterised over the role set. The deadline-emits case must NOT be silently unbuildable. |
| CONCERN | Risk & Robustness (Operator) | Infrastructure failure and honest abstention are indistinguishable on the only channel the plan guarantees travels with the fact. Both a missing `anthropic` client and a genuine speaker-never-settled-it produce `status = indeterminate` → tag `res:indeterminate`. `degraded=True` distinguishes them, but it lives **only in the sidecar** — and Risk 1's whole mitigation is that the sidecar must not be needed. So Error State Rendering's claim that "a downstream reader can always distinguish 'the model said it could not resolve this' from 'the resolution stage never ran'" is false on the tag-only retrieval path. | | Split the vocabulary at the tag: emit `res:degraded` (not `res:indeterminate`) whenever `Resolution.degraded is True`, keeping `res:indeterminate` for a model-emitted abstention. Five literals instead of four; spike-2 already proved low-entropy `res:*` literals clear the M2 firewall, so the added value costs nothing. Assert both tags in the Failure Path tests. |
| CONCERN | Risk & Robustness (Operator) + Scope & Value | The default install writes unbounded zero-information state. `M4_RESOLUTION_ENABLED` defaults **True**, `anthropic` is an *optional* dependency, and the sidecar has **"No TTL, matching M3's decision log"** — so every popoto install that enables the auditable path without `anthropic` writes one `ResolutionRecord` per accepted candidate, forever, each carrying `degraded=True` and nothing else. Retention is deferred to M9 (#568), which is open. | | Skip the sidecar write when `resolution.degraded is True` **and** `resolution.references` is empty — there is no evidence to store, and the `res:degraded` tag (previous finding) already records the state on the entry. Guard it in `_append_and_transition` next to the write call, not inside `ResolutionLog.write`, so the "written after a successful append so it can carry `entry_id`" ordering is unaffected. |
| CONCERN | Structural | Five Verification grep rows are mechanically broken: they pass `-E` (ERE) with **escaped** alternations (`"evidence_gap\|indeterminate\|…"`), which matches a *literal* pipe character. Verified empirically: `grep -c -E "a\|b"` returns `0`/exit 1 on a file containing `a` and `b`. Consequence: "All four statuses tested" (expects `> 3`) **always fails**, and the four anti-criterion rows (expect `0`) are **permanently false-green** — the #588 absorb-pattern, entity-graph, date-parser and numeric-confidence guards would never fire. | | Drop the backslashes under `-E` (`grep -cE "evidence_gap\|indeterminate"` → `grep -cE "evidence_gap|indeterminate"`), and note that `grep -c` over **multiple files** prints one `path:count` line per file, not a single number — so the multi-file rows ("No conflict-absorbing retry", "No date-parser dependency", "No numeric confidence field") must use `grep -chE … \| paste -sd+ - \| bc` or simply `! grep -qE …` with an exit-code expectation. |
| CONCERN | Structural + Scope & Value | Task 2's "Update `tests/benchmarks/test_defaults_sync.py` in the same commit" is underspecified for the test that actually gates. `test_all_defaults_covered_by_module_constants` asserts every uppercase `Defaults` attribute appears in `MODULE_CONSTANTS` **or** in the in-test `field_kwargs_and_class_attrs` exemption set — so ten new `M4_*` attributes fail it unless explicitly listed, and the plan never says which of the two lists they belong in. | | These are pinned magic numbers with no module-level alias (same shape as `M3_ASSEMBLY_CLAIM_TTL_MS`), so all ten go in `field_kwargs_and_class_attrs` in `tests/benchmarks/test_defaults_sync.py`, **not** in `tests/benchmarks/overrides.py::MODULE_CONSTANTS` — and each needs the one-line "why it is not sweepable" comment the neighbouring entries carry, or the file's own convention is broken. |
| CONCERN | Scope & Value (User) | `ExtractedFact.text` silently changes meaning while `span_start`/`span_end` do not. The current docstring (`extraction/__init__.py:55-57`) defines the span as offsets "of the source span in the turn text", and today `text == turn_text[span_start:span_end]`. After M4 `text` is the *rewrite* and only the new `verbatim` satisfies that identity — a user-facing contract change on a shipped dataclass, listed in Architectural Impact as merely "deliberate" with no consumer-facing callout. | | State the new invariant in the dataclass docstring explicitly — `verbatim == turn_text[span_start:span_end]`; `text` is the resolved form and is **not** span-addressable — and add it to the Documentation section's inline-docs checklist. Add one assertion in `tests/test_extraction.py` pinning the invariant on `verbatim` so a future refactor cannot re-point the offsets at `text`. |
| CONCERN | Scope & Value (Simplifier) | `ResolutionRecord` duplicates six fields the journal entry already owns (`status`, `statement`, `verbatim`, `speaker`, `captured_at`, `valid_from`) with no writer-of-record rule, so the two copies can disagree — and Race 2 deliberately allows the append to succeed while the sidecar write fails, which is exactly the state that makes divergence undetectable. | | Name the journal entry the record of truth for those six in the `ResolutionRecord` docstring, and keep them on the sidecar only as a denormalised read convenience for M7 (#566) — or drop `statement`/`verbatim`/`speaker`/`captured_at` from the model and let M7 join on `entry_id`, which the sidecar already stores. Do not add a reconciliation path; a sweep for divergence is M9's (#568) job, not M4's. |
| NIT | Structural | Prerequisite "Test DB pinned (not 15, not 0)" fails as written in a clean shell: this repo pins the test DB via `popoto_test_db = "15"` in `pyproject.toml`, not via `POPOTO_TEST_DB`, so the env var is unset by default and the check exits 1 before any work begins. | | Reword the Purpose column to say the check is a *per-run action item*, not a preexisting condition — the validator must export `POPOTO_TEST_DB=<free db>` first. |
| NIT | Structural | Freshness Check pins baseline `bb38f42` as `git rev-parse main`; `main` is now `ab4762c`. No cited claim moved (all file:line references re-verified during this critique), but the stated baseline is stale. | | Re-stamp the baseline commit when the plan is next revised. |
| NIT | Scope & Value (Simplifier) | Task 4 threads `resolution=None` through `_reconcile_pending` (`decision_log.py:697`), which only runs an identity query and a terminal write — it never calls `journal.append` and has no use for a resolution. | | Drop `_reconcile_pending` from the widening list; only `assemble` and `_append_and_transition` need the parameter. |
| NIT | Structural | Test Impact overstates the `_FakeJournal` change: `append(self, **kwargs)` already accepts arbitrary keywords and already records them via `self.appends.append({**kwargs, …})` (`tests/test_auditable_extraction.py:171-186`), so `captured_at`/`at` are assertable today with no edit. | | Reduce that bullet to "add assertions on the new kwargs"; no double update is required. |
