...
- **Connection pooling (added during build, commit `9b759db`)**: The pubsub listeners add one sync connection per model class, and an async stress test surfaced a pre-existing `MaxConnectionsError` from redis-py's effectively unbounded default pools. Both the sync and async global connections were switched to a bounded `BlockingConnectionPool` (default 128, overridable via `POPOTO_SYNC_MAX_CONNECTIONS` / `POPOTO_ASYNC_MAX_CONNECTIONS`) so excess concurrency queues for a free connection instead of erroring. This changes connection behavior for *all* Popoto operations, not just embeddings — documented in `docs/configuration.md`.
