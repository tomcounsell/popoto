...
3. **on_save() hook**: `IndexedFieldMixin.on_save()` fires for each indexed field:
   - Checks `_saved_field_values` for old value; if changed, removes old index entry via `SREM`
   - If `unique=True`, checks existing set at new value key via `SCARD`; raises if non-empty and not self
   - Adds `model.db_key.redis_key` to Set at `$IdxF:Model:field_name:value` via `SADD`
   - All operations pipelined -- zero extra round-trips
...
3. **filter_query()**: Executes `SMEMBERS $IdxF:Model:status:active` -- returns set of redis_keys
...
- Index key pattern: `$IdxF:Model:field_name:value` -> Set of redis_keys (mirrors `$KeyF` pattern)
- The `$IdxF` prefix is auto-generated by `FieldBase` metaclass via `field_class_key`
...
- `filter_query()` follows KeyFieldMixin's implementation pattern but uses `$IdxF` prefix instead of `$KeyF`
- Uniqueness check uses `SCARD` on the target Set key inside the pipeline; if count > 0 and the member is not self, raise before execute
...
**Impact:** Two concurrent saves with the same unique indexed value could both pass the SCARD check before either executes SADD.
**Mitigation:** This is the same limitation as KeyFieldMixin's uniqueness -- Redis does not support cross-key transactions. Document that uniqueness is best-effort under concurrent writes, same as existing `UniqueKeyField`. For strict uniqueness, use KeyField (which makes the value part of the Redis key, giving true uniqueness via key overwrite semantics).

## Race Conditions

### Race 1: Concurrent uniqueness check for indexed unique fields
**Location:** `IndexedFieldMixin.on_save()` -- SCARD check followed by SADD
...
**State prerequisite:** Both SCARD checks execute before either SADD
**Mitigation:** Same trade-off as existing `UniqueKeyField` -- application-level uniqueness, not database-level. Document this explicitly. For strict uniqueness under concurrency, recommend KeyField (key-level uniqueness).
...
- [ ] Code comments on uniqueness check explaining the race condition trade-off
...
- Implement `on_save()`: maintain Set at `$IdxF:Model:field_name:value`, handle old value cleanup via `_saved_field_values`
...
- Add uniqueness check in `on_save()` when `unique=True`: SCARD on target Set, raise `ModelException` if occupied by another instance
...
- Verify index key patterns in Redis: `redis-cli KEYS '$IdxF:*'`
