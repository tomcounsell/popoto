...
### Round 3 (2026-09-04, working tree `e042ee4`) — verdict: NEEDS REVISION

Depth: FULL (3 lenses). All findings reproduced directly against working-tree source before being
recorded. Re-verified clean: the narrowed inventory grep returns exactly **11** rows (round-2's
BLOCKER fix holds); all 11 site anchors match at their cited lines and enclosing symbols;
`_saved_field_values` does hold **decoded** Python objects (`encoding.py:505` builds it from
`getattr`), so `canonical_key_str` is effective at sites 2/3; no partition rendering exists on any
async path (`sorted_field_mixin.py` has no `async def`, and query.py's async entry points delegate);
`cyclic_decay_field.py:394-435` and `context_assembler.py:749-760` derive companion hashes from the
partition ZSET key and so converge post-fix with no edit; `confidence_modulation_args`
(`decaying_sorted_field.py:443-446`) routes both branches through ConfidenceField's own key
builders, so it adds no twelfth site.

| Severity | Critic | Finding | Implementation Note |
|----------|--------|---------|---------------------|
| BLOCKER | risk-robustness | The Risks item *"Orphan recovery differs by field type"* clause 1 — "Sorted-set orphans are recoverable — `Model.clean_indexes()` scans all five index types including sorted fields" — is **false**, and task 8 instructs BUILD to publish it in `CHANGELOG.md`. `clean_indexes` only removes index members "that reference instance keys which no longer exist in Redis" (`base.py:3712-3717`, `_collect_orphans` pipelines `EXISTS` and keeps only misses). After this change a legacy-keyed partition ZSET still holds members whose hashes are **alive**, so the scan discovers the key (pattern `base_key + "*"`, `base.py:3855-3857`) and removes **nothing**. The stale ZSET persists indefinitely as a shadow index. | Rewrite clause 1: sorted-set partition ZSETs orphaned by this change are **not** recoverable via `clean_indexes()` — its orphan test is `EXISTS` on the member's hash, which passes for live rows. The recovery path for all three field types is the same manual one clause 2 already gives: `SCAN` the legacy partition keys and `DEL` them after re-saving the affected rows (a re-save writes the canonical ZSET via `on_save`). Do not promise an automatic cleaner that does not exist — this is the #476 lesson the plan itself invokes. |
| CONCERN | scope-value + history-consistency | The plan converts three field families but documents only one. `## Documentation` targets `docs/fields.md:1205` (`## partition_by`) and `docs/multi-tenancy.md:78` — both **SortedField** sections. The docs that actually describe the two key formats this plan changes outside `DB_key` are untouched: `docs/fields.md:1970` + `### Partitioned Streams` (:1988) documents the EventStream key as `stream:{name}:{value}` with the literal example `stream:mutations:acme`, and `docs/multi-tenancy.md:123` `## Hash-based field partitioning (ConfidenceField)` documents the companion-hash partition. Task 8's gate (`grep -c -i "canonicaliz" docs/fields.md docs/multi-tenancy.md` each ≥ 1) is satisfied by the SortedField note alone, so the gap is invisible — the same "grep returns zero while the site remains" failure this plan names as its own top risk, transposed to docs. | Add the canonicalization one-liner at `docs/fields.md` `### Partitioned Streams` (~:1988) and `docs/multi-tenancy.md` `## Hash-based field partitioning (ConfidenceField)` (~:123), and strengthen the task-8 gate to require a hit in each of the four sections rather than one per file (e.g. `grep -c -i canonicaliz` ≥ 2 in each of the two files). |
