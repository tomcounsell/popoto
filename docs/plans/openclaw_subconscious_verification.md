...
|----------|--------|---------|--------------|---------------------|
| BLOCKER | History & Consistency (r2, elevated in aggregation) | Test Impact claims the shared fixture parametrizations need no change because they are name-driven. They are name-driven but not content-neutral: `tests/test_integrations_hooks.py:153` asserts `"health checks" in event.text`, `:166` asserts `"automatic rollback" in event.text`, and `:154` asserts `event.cwd == "/Users/dev/src/demo"` for every fixture. A live capture cannot produce that synthetic cwd — `cwd` is `ctx.workspaceDir`, a real path (the probe saw `/Users/valorengels/.openclaw-popotoprobe/workspace`) — so the byte-for-byte-capture Success Criterion and this assertion contradict each other. | Test Impact + Task 2 (capture-fixtures) | Script the capture prompt so both text substrings hold for the new fixtures. For cwd, add a per-fixture `CWDS` dict on the existing `TURN_IDS` pattern (three doc-derived fixtures keep the demo path, two OpenClaw fixtures carry the captured workspace path). Do NOT relax to `assert event.cwd` — a truthiness check is unfalsifiable and makes the required mutation proof vacuous. |
| CONCERN | Risk & Robustness (r1, C1) | `POPOTO_MEMORY_BIN` is the seam the failure-path test uses, but task 1 never required the plugin to read it and no existing convention establishes it. | Data Flow recall step 3 + Task 1 | `const bin = process.env.POPOTO_MEMORY_BIN \|\| "popoto-memory"; execFile(bin, ["hook"], {timeout: <ms below 15000>}, cb)`. New Verification row. |
| CONCERN | Risk & Robustness (r1, C2) | The plugin reads only `event.prompt`, observed on one turn. A continuation or tool-result turn may populate only `event.messages`; no `_QUERY_FIELDS` name matches the plural, so recall would emit nothing silently. | Data Flow recall step 2 + Task 1 | `const prompt = event.prompt \|\| lastUserMessageText(event.messages) \|\| "";` taking the last user-role entry's text. |
| CONCERN | Risk & Robustness (r1, C3) | Architectural Impact states reversibility for the repo, not for an operator who installed the plugin and set `allowConversationAccess=true`; combined with fail-silent design and no CI coverage, a regression degrades to silent no-memory with no documented off switch. | Documentation + Task 4 | Put `openclaw plugins uninstall popoto-memory` plus the config key to unset beside the existing `plugins inspect --runtime --json` check, so one section answers both "is it working?" and "how do I turn it off?". |
| CONCERN | History & Consistency (r1, C4) | The Inline Documentation task told the builder to comment "the `assistantTexts` join" — a call the Technical Approach forbids and the `grep -c "join("` == 0 anti-criterion rejects. Stale wording from the reversed design. | Documentation > Inline Documentation | Replace with "passing `assistantTexts` through unjoined — the adapter, not the plugin, reduces it". |
