...
### Critique Round 2 (2026-09-07)

War room run 2026-09-07, FULL depth, independent roster (3 critics: Risk &
Robustness, Scope & Value, History & Consistency); 3/3 returned, all grounded.
Verdict: **NEEDS REVISION** (1 blocker, 4 concerns, 0 nits). Every measured
claim below was reproduced by the aggregator, not relayed on the critic's word.
Environment: macOS 25.6 (APFS), Python 3.12.14, repo `.venv` setuptools 81.0.0,
`build` frontend absent from `.venv`, no Redis involved.

| Severity | Critic | Finding | Addressed By | Implementation Note |
|----------|--------|---------|--------------|---------------------|
| BLOCKER | Risk & Robustness (converged with Scope & Value, which flagged the same script from the complexity side) | Task 5a item 5 tells the script to choose the isolation branch from "the *measured* setuptools version of the venv that will run the build, using the Prerequisites-table command" — but item 4 creates that venv with `python3 -m venv` and installs only `build`, and **`build` does not depend on `setuptools`**. Reproduced on this machine: a fresh `python3.12 -m venv` plus `pip install build` yields exactly `build 1.6.0`, `packaging 26.3`, `pyproject_hooks 1.2.0`, `pip 26.2.1` — `import setuptools` raises `ModuleNotFoundError` both before and after the install. Under item 1's mandated `set -eu`, the item-5 probe therefore aborts the script before item 6 ever builds anything, so the script cannot satisfy its own Success Criterion ("`scripts/verify/sdist_excludes_tests.sh` runs to completion on this machine"), and both anti-criterion Verification rows — the plan's central proof that the sdist ships no tests — fail for a tooling reason rather than a membership reason. This is the round-1 BLOCKER's failure class returning in the mechanism written to fix it; the one mitigating difference is that the revised exit-code-checked rows make it a loud false *failure* rather than round 1's silent false *pass*. | Task 5a items 4-5, Prerequisites table, Success Criteria, Verification rows "Sdist ships no tests" / "Sdist check is warning-free" | Two viable fixes; pick one and write it into item 5 rather than leaving the referent ambiguous. (a) Install both: `"$TMP/venv/bin/python" -m pip install --quiet build setuptools`, then probe that venv. (b) Better — recognize that on the isolated branch the throwaway venv's setuptools is irrelevant, because `python -m build` provisions its own build env from `[build-system] requires`; probe the **outer** interpreter instead (`.venv`, measured 81.0.0) and treat "setuptools not importable" as `0` (take the isolated branch) rather than as an error: `python -c "import sys;\ntry:\n import setuptools\n from packaging.version import Version\n sys.exit(0 if Version(setuptools.__version__) >= Version('83') else 1)\nexcept ImportError:\n sys.exit(1)"`. Whichever is chosen, item 5 must name *which interpreter* runs the probe — the current wording ("the venv that will run the build") reads as the throwaway venv, which is the unrunnable reading. |
| CONCERN | History & Consistency | The Verification row "Checker comment no longer frames the allowlist as observed 1.9.0 membership" runs `grep -c 'the 1.9.0 sdist has' scripts/check_sdist_contents.py` expecting `0` — but the comment at `scripts/check_sdist_contents.py:46` actually reads `# The seven top-level entries **the published** 1.9.0 sdist has.`, so the literal substring is **already absent on unmodified `main`** (reproduced: `grep -n` exits 1, no match). The row prints its pass value before any work is done and can never catch a builder who skips Task 2's inline-comment edit. This is the #661 vacuity trap that the plan explicitly guards against elsewhere (it added a non-empty-range row for the brittle `awk` check) but missed for its own new row. The sibling `CLAUDE.md` row is fine — measured `1` on `main`, so it is non-vacuous. | Verification table, Task 2 (`build-manifest`), Documentation → Inline Documentation | Change the pattern to one that matches today's text: `grep -c 'published 1.9.0 sdist has' scripts/check_sdist_contents.py` → measured **1** on `main`, so it is a genuine red state that goes to `0` only when the comment is rewritten. (`grep -c '1.9.0 sdist has'` also measures `1` and is the looser alternative.) Re-measure the chosen pattern on unmodified `main` before accepting the row — a doc-cascade grep whose red state is not confirmed non-zero is not a guard. |
| CONCERN | History & Consistency | The revision added a fifth documentation-cascade item and updated Success Criteria to "All **five** documentation cascade items landed", but three other passages still say four. Reproduced — `grep -n "doc cascade is four\|all four are required\|four required items" docs/plans/sdlc-689.md` returns line 679 (No-Gos: "the doc cascade is four passages"), line 748 (section header: "the actual cascade — all four are required"), and line 933 (Task 4 `Informed By`: "the Documentation section's four required items"). A builder reading Task 4 in isolation is told to do four of five items, in a plan whose entire subject is prose that stops being true. | Documentation section header, No-Gos, Task 4 `Informed By` | Update all three to five. Acceptance check: `grep -c "doc cascade is four\|all four are required\|four required items" docs/plans/sdlc-689.md` → `0`. Task 4's body already enumerates the three CLAUDE.md clauses correctly, so only the counts are wrong; do not renumber the Documentation bullets. |
| CONCERN | Risk & Robustness | The row `sh scripts/verify/sdist_excludes_tests.sh && test "$(sh scripts/verify/sdist_excludes_tests.sh)" = "0"` invokes the whole clone → venv → **network-isolated** build cycle twice, and the `--run-checker` row triggers a third. On this machine setuptools 81.0.0 forces the isolated branch, so one verification pass costs three network builds, and a transient network failure on the second or third invocation reads as a regression in the fix rather than as infrastructure flake. | Verification table rows "Sdist ships no tests" / "Sdist check is warning-free", Task 5a | Capture once instead of re-invoking: `OUT=$(sh scripts/verify/sdist_excludes_tests.sh) && test "$OUT" = "0"` preserves the exit-code check the revision added (`$( )` failing under `set -e` still aborts, and an aborted script yields an empty `OUT` which fails the `test`) while halving the builds. This is a cost/flake concern, not a correctness one — item 1's `set -eu` already prevents the silent false pass — so it may be accepted on the record if the appetite is spent, but say so rather than leaving the triple build unremarked. |
| CONCERN | Scope & Value | The revision answered round 1's "heavy for a Small appetite" concern by *growing* the script: Task 5a is now a 9-item shell-program specification (trap cleanup, HEAD-equality clone guard, on-demand venv resolution, measured-version isolation branch, a two-mode CLI with its own flag-validation contract). Its three siblings in `scripts/verify/` are one-line `grep -c` invocations (`no_new_deps.sh` is three lines including the shebang and comment), and this script is wired into no CI job and no `release.yml` step — its only consumer is this PR's Verification table, after which it becomes permanent unexercised maintenance surface for a one-directive fix. | Task 5a, Appetite, Team Orchestration | If kept, state its lifecycle explicitly in Task 5a: it is a repo-committed one-shot validator that no CI job runs, and a later editor may delete it without replacing anything. If that is not acceptable, either wire it into a CI job so it stays honest, or demote it to a scratch script pasted into the PR description and inline the clone-build-count into the two Verification rows — do **not** delete the rows along with the script, since they carry the plan's central anti-criterion. |

**Structural check results (round 2):** required sections PASS; task numbering
PASS (1, 2, 3, 4, 5, 5a — no gaps); dependency references PASS (all resolve, no
cycles); file paths PASS (`MANIFEST.in` and `scripts/verify/sdist_excludes_tests.sh`
are intentionally new, every other cited path exists); prerequisites FAIL as the
plan itself measures and accommodates (`build` absent from `.venv`, setuptools
81.0.0 < 83); cross-references FAIL (the four/five cascade count above, and the
vacuous checker grep row above).
