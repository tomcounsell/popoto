...
partition, and since 1.9.0 so does `inject_context`'s default retrieval path
(lexical/BM25) ([#576](https://github.com/tomcounsell/popoto/issues/576)).
On 1.8.2 and earlier, two agents sharing one Redis at Level 0 could retrieve
each other's memories; a distinct Redis database per agent was the workaround
and is still the boundary Redis itself enforces.
