...
`popoto.get_redis()` is rebind-safe; `popoto.POPOTO_REDIS_DB` is **not**. `set_REDIS_DB_settings()` rebinds `redis_db`'s module global, and `src/popoto/__init__.py` holds an import-time copy of that name, so the re-exported `popoto.POPOTO_REDIS_DB` still points at the pre-reconfiguration client. `get_redis()` delegates to `redis_db.get_REDIS_DB()` on every call (#645) and therefore does not. The pytest plugin's `_swap_db()` is immune to this either way — it mutates the pool on the existing object rather than rebinding the name.
